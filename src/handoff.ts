...
context-timeline.md -> history-timeline.md -> history-events.md

1. 先读 \`context-timeline.md\`，它是 provider 切换后的主接续材料。
2. 需要理解完整工作流时，读 \`history-timeline.md\`，它保留所有关键事件的 \`event_ref\`。
3. 只有需要追查具体命令输出、完整 diff、MCP 或动态工具细节时，再读 \`history-events.md\` 或用 \`event_ref\` 回查 \`source.jsonl\`。
