...
- 用户给了 `context-timeline.md`、`context-events.md`、`history-timeline.md`、`history-events.md`、`source.jsonl`
...
4. 需要理解完整工作流时，再读 `history-timeline.md`。
5. 只有遇到具体 `event_ref`，或需要追查更早的命令输出、完整 diff、MCP/动态工具细节时，才读 `history-events.md` 或回查 `source.jsonl`。
6. 第一轮 turn 结束时，只汇报“已经恢复到什么状态、还缺什么、下一步建议怎么做”，不要直接开始实现。
...
- 一开始全文读 `history-events.md`
  应先读 `context-timeline.md` 和 `context-events.md`；`history-events.md` 是完整审计兜底，不是第一阅读材料。

- 只复制 Markdown，不复制 `source.jsonl`
  这样接续包离开原机器路径后，`event_ref` 回查会变脆弱。

- 把 `history-timeline.md` 当作当前 prompt
  它是工作流索引；真正的接续主材料是 `context-timeline.md` 和 `context-events.md`。
