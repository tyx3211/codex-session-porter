...
原因是本工具会直接读取 Codex 的 SQLite state DB（状态数据库），实现使用了 Node 内置 `node:sqlite`。低版本 Node 可能没有该模块，或者行为不一致。
...
This tool reads the Codex SQLite state DB directly through Node's built-in `node:sqlite` module. Older Node versions may not provide that module or may behave differently.
